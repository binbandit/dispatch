...
export function SheetBackdrop({
  ...
  return (
    <SheetPrimitive.Backdrop
      className={cn(
        "fixed inset-0 z-50 bg-black/32 backdrop-blur-sm transition-[opacity,backdrop-filter] duration-200 data-ending-style:opacity-0 data-starting-style:opacity-0",
        className,
      )}
    ...
    <SheetPortal>
      ...
      >
        <SheetPrimitive.Popup
          className={cn(
            "bg-popover text-popover-foreground relative flex max-h-full min-h-0 w-full min-w-0 flex-col overscroll-contain shadow-lg/5 transition-[opacity,translate] duration-200 ease-in-out will-change-transform not-dark:bg-clip-padding before:pointer-events-none before:absolute before:inset-0 before:shadow-[0_1px_--theme(--color-black/4%)] data-ending-style:opacity-0 data-starting-style:opacity-0 max-sm:before:hidden dark:before:shadow-[0_-1px_--theme(--color-white/6%)]",
            side === "bottom" &&
